...
exports.liveCommand = new commander_1.Command('live')
    ...
    const server = app.listen(options.port, () => {
        ...
        console.log(chalk_1.default.green(`URL      : http://localhost:${options.port}`));
        console.log(chalk_1.default.yellow('\nEn attente de commandes de pilotage...'));
    });
    server.on('error', (err) => {
        if (err.code === 'EADDRINUSE') {
            console.error(chalk_1.default.red(`❌ Le port ${options.port} est déjà utilisé. Essayez un autre port avec --port <port>.`));
        }
        else {
            console.error(chalk_1.default.red(`❌ Erreur lors du démarrage du serveur : ${err.message}`));
        }
        process.exit(1);
    });
    ...
    app.post('/pilot', (req, res) => {
        ...
        try {
            ...
            if (action === 'update_variable') {
                ...
                console.log(chalk_1.default.green(`✅ Variable mise à jour : ${data.name} = ${data.value}`));
 ...
 */
function updateVariable(filePath, name, value) {
    const xmlData = fs.readFileSync(filePath, 'utf8');
    const parser = new fast_xml_parser_1.XMLParser({ ignoreAttributes: false });
    const builder = new fast_xml_parser_1.XMLBuilder({ ignoreAttributes: false, format: true });
    const jsonObj = parser.parse(xmlData);
    if (!jsonObj.config)
        jsonObj.config = { config: [] };
    let configs = Array.isArray(jsonObj.config.config) ? jsonObj.config.config : [jsonObj.config.config];
    let variablesConfig = configs.find((c) => c['@_key'] === 'workflow_variables');
    if (!variablesConfig) {
    ...
    if (existingVar) {
        ...
        if (!Array.isArray(variablesConfig.config))
